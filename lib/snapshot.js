...
async function poolSnapshot() {
  ...
  if (Math.abs(Date.now() / 1000 - Number(block.timestamp)) > 120)
    throw new Error("Chain snapshot is stale.");
  ...
  const base = reserves[bnbFirst ? 0 : 1],
    quote = reserves[bnbFirst ? 1 : 0];
  if (base === 0n || quote === 0n) throw new Error("Empty pool.");
  const value = {
    kind: "onchain_pool_snapshot",
    chainId: 56,
    network: "BSC mainnet (read only)",
    venue: "PancakeSwap V2 WBNB/USDT",
    blockNumber: String(block.number),
    blockTime: new Date(Number(block.timestamp) * 1000).toISOString(),
    fetchedAt: new Date().toISOString(),
    pool: pair,
    explorer: `https://bscscan.com/address/${pair}`,
    price: Number(
    ...
    decimals: 18,
    reserveUpdatedAt: new Date(Number(reserves[2]) * 1000).toISOString(),
    warning:
      "Pool reserve ratio, not an oracle or executable quote. All other analysis inputs remain user supplied.",
  };
