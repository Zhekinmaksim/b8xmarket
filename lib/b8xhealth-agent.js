...
function getInvokeUrl(req) {
  return `${getOrigin(req)}/api/agents/b8xhealth/a2a`;
}
...
function dataPart(req) {
  ...
  const part = message && Array.isArray(message.parts)
    ...
    : null;
  return (part && part.data) || body || {};
}

function rpcResponse(id, data) {
  return {
    jsonrpc: "2.0",
...
function negotiate(req, payload) {
  ...
  return rpcResponse(req.body && req.body.id, {
    status: "quoted",
    agent: AGENT.name,
    wallet: AGENT.wallet,
    quote: {
...
function handleA2a(req, res) {
  ...
  if (payload.skill === "negotiate") return sendJson(res, 200, negotiate(req, payload));
  if (payload.skill === "notify_funded") return sendJson(res, 200, notifyFunded(req, payload));
...
function handleAgent(req, res) {
  if (req.method === "GET") return sendJson(res, 200, agentCard(req));
  return handleA2a(req, res);
}

function handleCard(req, res) {
  if (req.method !== "GET") return sendJson(res, 405, { error: "method_not_allowed" });
  return sendJson(res, 200, agentCard(req));
}
