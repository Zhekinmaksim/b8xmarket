async (page) => {
  ...
  for(const slug of ['b8xrebal','b8xgrid','b8xyield','b8xhealth']) {
    ...
    await page.locator('#run-status').filter({hasText:/Completed in/}).waitFor();
    checks.push({slug,decision:await page.locator('#decision').innerText(),rows:await page.locator('#result-table tbody tr').count()});
  }
  ...
  await page.reload();
  if(await page.locator('[data-result]').count()!==5)throw new Error('History did not persist');
  ...
  await page.getByRole('button',{name:'Run analysis',exact:true}).click();
  await page.locator('#run-status').filter({hasText:'upper must be greater than lower'}).waitFor();
  ...
  await page.locator('#source-note').filter({hasText:/at BSC mainnet block/}).waitFor({timeout:25000});
  await page.getByRole('button',{name:'Run analysis',exact:true}).click();
  await page.locator('#result-source').filter({hasText:/PancakeSwap V2/}).waitFor({timeout:25000});
  if(errors.length)throw new Error(errors.join('\n'));
  console.log(JSON.stringify({checks,errors,mobileOverflow:overflow,download:download.suggestedFilename(),liveSource:await page.locator('#result-source').innerText()}));
}
