...
async function main() {
  ...
  for (const agent of catalog) {
    const input = {
      ...Object.fromEntries(agent.fields.map((f) => [f.key, f.value])),
      ...(agent.routes ? { routes: agent.routes } : {}),
    };
    const samples = [];
    for (let sample = 0; sample < 5; sample++) {
      const before = performance.now(),
        withoutMarketplace = baseline(agent.slug, input),
        baselineMs = performance.now() - before;
      const start = performance.now();
      const res = await fetch(`${origin}/api/agents/${agent.slug}/a2a`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          params: {
            message: {
              role: "user",
              messageId: `benchmark-${agent.slug}-${sample}`,
              parts: [{ kind: "data", data: { skill: "analyze", input } }],
            },
          },
        }),
      });
      ...
      if (!res.ok || body.error)
        throw new Error(`${agent.slug}: ${JSON.stringify(body)}`);
      const report = body.result.parts[0].data;
      const checks = Object.entries(withoutMarketplace).map(
        ([key, expected]) => ({
          key,
          expected,
          actual: report.result.metrics[key],
          passed:
            typeof expected === "number"
              ? Math.abs(expected - report.result.metrics[key]) <=
                Math.max(1e-8, Math.abs(expected) * 1e-10)
              : expected === report.result.metrics[key],
        }),
      );
      if (checks.some((c) => !c.passed))
        throw new Error(`Reference mismatch for ${agent.slug}`);
      samples.push({
        sample,
        baselineMs,
        roundTripMs,
        baselineOutput: withoutMarketplace,
        checks,
        report,
      });
    }
    ...
    const item = {
      ...
      summary: {
        runs: samples.length,
        p50RoundTripMs: sorted[2],
        maxRoundTripMs: sorted[4],
        p50BaselineMs: samples
          .map((s) => s.baselineMs)
          .sort((a, b) => a - b)[2],
        checksPassed: samples[0].checks.length,
        checksTotal: samples[0].checks.length,
      },
    };
    tasks.push(item);
    fs.writeFileSync(
      path.join(output, `${agent.slug}.json`),
  ...
  const data = {
    ...
    mode: "deterministic scenarios; HTTP analysis versus local standalone non-agent calculator",
    limitations: [
    ...
    tasks: tasks.map((t) => ({ agent: t.agent, ...t.summary })),
  };
  fs.writeFileSync(
  ...
  const md = [
    ...
    ...data.tasks.map(
      (t) =>
        `| ${t.agent} | ${t.p50RoundTripMs.toFixed(2)} | ${t.maxRoundTripMs.toFixed(2)} | ${t.p50BaselineMs.toFixed(4)} | ${t.checksPassed}/${t.checksTotal} |`,
    ),
    "",
