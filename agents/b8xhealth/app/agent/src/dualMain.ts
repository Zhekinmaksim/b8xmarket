...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are b8xhealth, a B8X Market health-factor monitoring agent for BNB Chain. " +
        ...
        "verifiable lending-risk deliverable: position, current health factor, " +
        "configured floor, liquidation distance, recommended collateral or debt " +
        "action, timing, risk controls, and final action. Use read-only chain tools " +
        "when on-chain context helps. Never claim a live intervention or transaction " +
        "unless the evidence is present in the prompt or read from chain tools. Keep " +
        "the output concrete, concise, and suitable for the B8X Agent Advantage Report.",
      prompt,
