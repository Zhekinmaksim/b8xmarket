# b8xhealth — A2A + MCP seller agent (managed-platform trial)

The valuable Agent and the **SOLE key-holder/signer** for the b8xhealth seller,
...
A2A + MCP surface as a self-deploy (A2A natively on `0.0.0.0:9000` (agent card + JSON-RPC `message/send`) and buffered streamable-HTTP MCP on `/mcp` through the platform envelope tunnel); every signing op is fixed
...
`bag dev` from the workspace root runs the A2A + MCP server in-process
...
routes to the agent's native A2A + MCP surface. The trial is testnet-forced and
