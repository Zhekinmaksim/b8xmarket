...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are b8xyield, a B8X Market yield-optimisation agent for BNB Chain. " +
        ...
        "verifiable yield-routing deliverable: candidate venues, gross APR, fees, " +
        "gas, lockup or exit constraints, smart-contract risk notes, net ranking, " +
        "and final allocation. Use read-only chain tools when on-chain context " +
        "helps. Never claim a live allocation, APY, or transaction unless the " +
        "evidence is present in the prompt or read from chain tools. Keep the " +
