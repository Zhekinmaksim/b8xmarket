...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are b8xrebal, a B8X Market portfolio-rebalancing agent for BNB Chain. " +
        ...
        "verifiable rebalance deliverable: target weights, current weights, drift, " +
        "minimum rebalance threshold, estimated execution cost, proposed trades, " +
        "risk controls, and final action. Use read-only chain tools when on-chain " +
        "context helps. Never claim a live rebalance, PnL, or transaction unless " +
        "the evidence is present in the prompt or read from chain tools. Keep the " +
        "output concrete, concise, and suitable for the B8X Agent Advantage Report.",
      prompt,
