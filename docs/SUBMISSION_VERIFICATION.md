...
Observed 2026-09-08 against https://b8xmarket.xyz.
...
`bag deploy prepare --provider aws` is not ready: no real AWS account/region is configured, and the LLM key and storage upload endpoint are missing. The grid wallet has approximately 0.050385 tBNB and 0 U. Zero-price ERC-8183 is configured; no testnet job was funded or delivered during these checks.

Permanent hosting and any signer secret handoff require an explicit provider decision. The public Vercel application has no signer and must remain keyless.

## Claims not established

On 2026-09-09, the deploy-contained grid seller built successfully and passed all five focused tests, including validation before signing, report delivery using mocked chain operations, and rejection without fake submission. This implementation is not connected to the public analysis deployment and has not delivered an on-chain job.

This verification does not establish paid hiring, strategy execution, continuous monitoring, Altana session transactions/revocation, human time savings, or investment returns. A local build or mock-signing test is not on-chain delivery evidence.
